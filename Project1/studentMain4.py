...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    if OTHER == None:
        ...
        for i in range(N):
             t1 = random.gauss(pi,pi)
                         
             x1 = target_measurement[0]+ random.gauss(0,distance_noise_factor*random.gauss(2.5,2.5)) #random.gauss(2.5,2.5)random.gauss(0,distance_noise_factor*random.uniform(1,5))
                     
             y1 = target_measurement[1]+ random.gauss(0,distance_noise_factor*random.gauss(2.5,2.5))#random.gauss(0,distance_noise_factor*random.gauss(2.5,2.5))
        ...
        OTHER = [[0.0,0.0], thetha,p_initial,step,[0.0,0.0,0.0],thetha_list,last_turn, turning_list,turn_list]
    ...
    turn2 = turn2 + random.gauss(0, distance_noise_factor*distance2)
    ...
    #print(turn_avg, 'turn average', turn2, 'turn2 is')
    ...
    for i in range(look_ahead):
        ...
        target_ahead.move(turn_avg, distance2)
    ...
    #turning = turning + sqrt((random.gauss(0, distance_noise_factor)*2)**2)
    if turning < -pi:
    ...
    #print('the revised measurememnys are', position[0],position[1],position[2], turn2, distance2)
    #print('distance between target and hunter', distance_between(hunter_position,target_measurement))
    #print(xy_estimate, 'target will go to','and heading is', z)
    #print(heading_to_target, 'now the heading to target is')
    #print(turning, hunter_distance, max_distance, 'turn, dist, max')
    #print ('')    
    
    
    OTHER = [target_measurement,thetha,result,step,updated_measurement,thetha_list,last_turn,turning_list,turn_list]
...
def measurement_prob(predict, measurements,bearing_noise,m_thetha,std_mthetha,p_heading):
        ...
        #print('error meas', error_measurement, sigma, 'sigma')
        
        #error *= exp(- ((error_measurement) ** 2) / (sigma ** 2) / 2.0) / sqrt(2.0 * pi * (sigma ** 2))
...
#target = robot(2.1, 4.3, 0.5, 2*pi/34.0, 1.5)
